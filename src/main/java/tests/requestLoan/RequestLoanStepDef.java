...
public class RequestLoanStepDef extends BaseTest {
    ...
    public void getTheLoanRequestStatus(String loanRequestStatus) {
        paraBank.requestLoanPage.getLoanStatus(loanRequestStatus);
        Assert.assertEquals( paraBank.requestLoanPage.getLoanStatus(loanRequestStatus),"Approved");
//
    }

    @And("get  loan request result {string}")
    public void getTheLoanRequestResult(String loanRequestResult) {
        paraBank.requestLoanPage.getLoanResult(loanRequestResult);
        Assert.assertEquals(paraBank.requestLoanPage.getLoanResult(loanRequestResult),"Congratulations, your loan has been approved.");
